# CLAUDE.md — CatVox AI Developer Onboarding

This file is read automatically by Claude Code at session start. It captures the work style, process decisions, and architectural context established during this project's development.
...
├── TRD.md                         # Technical requirements (v1.8)
...
From `TODO.md` — use these when implementing the Cloud Function:
- **Temperature:** 0.7
...
- **HLD.md** — only stable design decisions (the "why"). No implementation status, no concrete values that will change.
- **TRD.md** — everything concrete: versions, bucket names, roles, backlog status.
...
- **No ads.** Monetization is StoreKit 2 IAP only (from PROMPT.md).
